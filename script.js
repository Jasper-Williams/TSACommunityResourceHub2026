document.addEventListener("DOMContentLoaded", function () {
    ...
    if (searchInput) {
        searchInput.addEventListener("input", function () {
            ...
            items.forEach(item => {
                const categories = (item.dataset.category || "").toLowerCase().split(",").map(cat => cat.trim());
    ...
    if (form) {
        form.onsubmit = function (event) {
            ...
            .then(() => { alert("Resource submitted successfully!"); form.reset(); })
